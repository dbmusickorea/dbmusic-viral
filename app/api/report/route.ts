...
export async function GET(request: NextRequest) {
  ...
  postsHeader.eachCell(cell => { cell.fill = headerFill; cell.alignment = { horizontal: 'center' }; cell.border = border })

  posts?.forEach(p => {
    const row = postsSheet.addRow([
      p.influencer_name, p.platform,
      { text: p.post_url, hyperlink: p.post_url },
      p.likes_count ?? 0, p.comments_count ?? 0, p.views_count ?? 0,
      p.is_cover ? '✅' : '',
  ...
  if (coverPosts.length > 0) {
    ...
    coverSheet.columns = [
      { header: '참여자', width: 15 }, { header: '플랫폼', width: 12 },
      { header: '게시물 링크', width: 45 }, { header: '좋아요', width: 12 },
      { header: '댓글', width: 12 }, { header: '승인상태', width: 12 }, { header: '등록일', width: 15 }
    ]
    const coverHeader = coverSheet.getRow(1)
    coverHeader.font = headerFont
    coverHeader.eachCell(cell => { cell.fill = headerFill; cell.alignment = { horizontal: 'center' }; cell.border = border })
  ...
  if (commentMissions && commentMissions.length > 0) {
    ...
    commentHeader.eachCell(cell => { cell.fill = headerFill; cell.alignment = { horizontal: 'center' }; cell.border = border })

    commentMissions.filter(m => m.project_code !== 'UNLOCK').forEach(m => {
      const row = commentSheet.addRow([
        m.youtube_handle, m.video_id,
