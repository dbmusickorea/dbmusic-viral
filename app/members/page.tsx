          ...
          <div key={p.id} className="border dark:border-gray-600 dark:bg-gray-700 rounded-lg p-3">
            <div className="flex justify-between items-start">
              <div>
                ...
                {(p.cover_video_count > 0 || p.premium_cover_video_count > 0) && <p className="text-xs text-purple-500 dark:text-purple-400 flex items-center gap-0.5"><Music size={10} /> 커버: {p.cover_current ?? 0}/{(p.cover_video_count ?? 0) + (p.premium_cover_video_count ?? 0)}{p.cover_video_count > 0 && p.premium_cover_video_count > 0 ? ` (일반 ${p.cover_video_count} + 프리미엄 ${p.premium_cover_video_count})` : ''}</p>}
              </div>
            ...
            {tab === 'participant' && (
              <div className="bg-white dark:bg-gray-900 rounded-2xl p-4 mb-4">
                <div className="mb-3">
                  ...
                  <div className="relative z-0 flex gap-1 p-1 bg-gray-100 dark:bg-gray-800 rounded-lg">
                    <div
                    ...
                    />
                    <button ref={el => { filterBtnRefs.current[0] = el }} onClick={() => setCoverFilter('all')} className={`relative z-10 flex-1 text-[10px] px-1 py-1 rounded-md whitespace-nowrap ${coverFilter === 'all' ? 'text-white' : 'text-gray-500 dark:text-gray-400'}`}>전체</button>
                      ...
                      <input type="checkbox" checked={selected?.is_agency ?? false} onChange={async (e) => {
                        ...
                        if (!checked) {
                          await fetchWithAuth(`/api/participants?id=${selected.id}`, {
                            method: 'PATCH',
                            headers: { 'Content-Type': 'application/json' },
                            body: JSON.stringify({ is_agency: false })
                          })
                          setParticipants((prev: any[]) => prev.map(p => p.id === selected.id ? { ...p, is_agency: false } : p))
                          showToast('에이전시 대표 해제됐어요.')
                        }
                      }} />
