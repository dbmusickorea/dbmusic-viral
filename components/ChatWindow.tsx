...
export default function ChatWindow({ userId, role, viewerType, title, subtitle, onBack, embedded = false }: Props) {
  ...
  return (
    <div className={`fixed ${embedded ? 'md:static' : ''} top-0 left-0 right-0 z-[60] ${embedded ? 'md:z-0' : ''} flex flex-col items-center bg-gray-50 dark:bg-gray-900 h-[100dvh] ${embedded ? 'md:h-full' : ''} w-full`}>
      ...
      <div
        className={`relative flex-1 w-full max-w-2xl ${embedded ? 'md:max-w-none' : ''} mx-auto overflow-hidden flex flex-col`}
      ...
      <div ref={scrollContainerRef} onScroll={handleScroll} className="chat-scroll flex-1 overflow-y-auto px-4 py-3 space-y-2">
        {loading ? (
        ...
        <div className="fixed inset-0 z-[75] bg-white dark:bg-gray-900 flex flex-col" style={{paddingTop: 'env(safe-area-inset-top)'}}>
          ...
          <div className="w-full md:max-w-7xl md:mx-auto p-3 shrink-0">
            <div className="relative flex p-1 bg-gray-100 dark:bg-gray-700 rounded-lg">
              ...
              />
              <button onClick={() => setGalleryTab('media')} className={`relative z-10 flex-1 py-1.5 text-xs rounded-md font-medium ${galleryTab === 'media' ? 'text-white' : 'text-gray-500 dark:text-gray-400'}`}>사진/동영상</button>
              <button onClick={() => setGalleryTab('files')} className={`relative z-10 flex-1 py-1.5 text-xs rounded-md font-medium ${galleryTab === 'files' ? 'text-white' : 'text-gray-500 dark:text-gray-400'}`}>파일</button>
              <button onClick={() => setGalleryTab('links')} className={`relative z-10 flex-1 py-1.5 text-xs rounded-md font-medium ${galleryTab === 'links' ? 'text-white' : 'text-gray-500 dark:text-gray-400'}`}>링크</button>
            </div>
          </div>
