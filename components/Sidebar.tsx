    ...
    <div className="fixed inset-0 z-50 flex">
      <div id="tutorial-sidebar-menu" className="bg-white dark:bg-gray-800 w-64 h-full shadow-xl p-6 flex flex-col">
        ...
        <div className="space-y-2 flex-1">
          {items.map((item, i) => (
            <button key={i} id={item.id} onClick={() => { item.onClick(); onClose() }} className={`w-full text-left px-3 py-3 rounded-lg text-sm font-medium ${item.active ? 'bg-blue-50 text-blue-600' : 'text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700'}`}>
              {item.label}
            </button>
          ))}
        </div>
